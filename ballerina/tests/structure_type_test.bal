...
type UnionWithArrays int[][]|float[]|string[][][]?;
...
@test:Config {}
public isolated function testUnionArrays() returns error? {
    UnionArray nums = [[[1],"true",()], []];
...
type L decimal[][]|string;
type M ();
type N L|M;
type O L|boolean[];
type UnionArray2 N[][]|O[];

@test:Config {}
public isolated function testUnionArrays2() returns error? {
    UnionArray2 nums = [[[[1.99999],[1.99999]],"true",()], []];

    Proto3Schema ser = check new (UnionArray2);
    byte[] encoded = check ser.serialize(nums);

    Proto3Schema des = check new (UnionArray2);
    UnionArray2 decoded = check des.deserialize(encoded);
    test:assertEquals(decoded, nums);
}


type Data decimal|byte|int|float|boolean|string|string[]|int[][]|float[][][][][]?;

@test:Config {}
public isolated function testUnionArrays3() returns error? {
    Data nums = 3.9d;

    Proto3Schema ser = check new (Data);
    byte[] encoded = check ser.serialize(nums);

    Proto3Schema des = check new (Data);
    Data decoded = check des.deserialize(encoded);
    test:assertEquals(decoded, nums);
}
